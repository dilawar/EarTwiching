...
import sys
import pandas as pd
import analyze_using_mean_stdusing_mean_std as method
try:
...
def process( trialdir ):
    ...
    resdir = os.path.join( trialdir, method.resdir_name_ )
    ...
    for i, (f, res) in enumerate(trial_data_):
        time, signal, lines = res
        d = linesToDataFrame( lines )
        print( d )
        quit()
        tvec = d['time']
        cs = d['cs']
